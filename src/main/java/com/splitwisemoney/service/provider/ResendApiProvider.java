...
/**
 * Email Provider using Resend HTTP REST API over HTTPS (Port 443).
 */
@Service("resendApiProvider")
public class ResendApiProvider implements EmailProvider {

    private static final Logger log = LoggerFactory.getLogger(ResendApiProvider.class);
    ...
    public void sendExistingUserInvitation(String toEmail, String inviterName, String groupName, String token, LocalDateTime expiresAt) {
        ...
        sendViaResendApi(toEmail, subject, html);
    }
    ...
    public void sendTestEmail(String toEmail) {
        String subject = "SplitWise Resend HTTPS API Test";
        String html = "<p>Test email sent via <strong>Resend HTTPS REST API</strong> over Port 443.</p>";
        sendViaResendApi(toEmail, subject, html);
    }

    private void sendViaResendApi(String toEmail, String subject, String htmlContent) {
        if (!isConfigured()) {
            throw new MailSendException("Resend API is not configured (RESEND_API_KEY missing)");
        }

        log.info("[ResendApiProvider] Sending email via Resend HTTPS API -> to={}", toEmail);
        ...
        try {
            ...
            throw new MailSendException("Resend HTTP API delivery failed: " + e.getMessage(), e);
        }
    }
